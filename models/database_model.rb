# 1. why Database::Model.database and not Database::Model.filename in rake file
# 2. what is * in line 73:  def self.where(query, *args)
...
module Database
  ...
  class Model
    ...
    def self.all
      Database::Model.execute("SELECT * FROM #{self.to_s.downcase.pluralize}").map do |row|
        self.new(row)
      end
    end
    ...
    def self.where(query, *args)
      Database::Model.execute("SELECT * FROM #{self.to_s.downcase.pluralize} WHERE #{query}", *args).map do |row|
        self.new(row)
      end
    end
